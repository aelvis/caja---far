...
@Component({
  selector: 'app-coreccion-descripcion-factura',
  templateUrl: './coreccion-descripcion-factura.component.html',
  styleUrls: ['./coreccion-descripcion-factura.component.css']
})
export class CoreccionDescripcionFacturaComponent implements OnInit {
	...
	showInfo(titulo,mensaje){
  	...
  	cambiarNombreItemController(id){
 	  	this._usu.cambiarNombreItemServiceFactura(id,this.enviar_nueva_descripcion).subscribe(
	  		res => {
	  			if(res["mensaje"].terminar){
					localStorage.clear();
					this._router.navigate(['/login']);
	...
	verificarAnulacionOperacionDos(verificar_dos_nuevo){
	  ...
	  this._usu.guardarAnulacionOperacioneBdServiceDevolucionTotakDescripcionFactura(this.ultima_boleta,this.descripcion_operacion_anulacion,this.buscar_anulacion_operacion).subscribe(
	  		res => {
	  			if(res["mensaje"].terminar){
					localStorage.clear();
					this._router.navigate(['/login']);
