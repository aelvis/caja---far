...
export class DevolucionItemComponent implements OnInit {
	...
	showInfo(titulo,mensaje){
  	...
  	cambiarNombreItemController(id,precio,representacion,producto_sucursal){
 	  	this._usu.actualizarPedidoTickService(id,this.enviar_nueva_descripcion,precio,representacion,producto_sucursal).subscribe(
	  		res => {
	  			if(res["mensaje"].terminar){
					localStorage.clear();
					this._router.navigate(['/login']);
	...
	verificarAnulacionOperacionDos(verificar_dos_nuevo){
	  ...
	  this._usu.guardarAnulacionOperacioneBdDevolucionTotalDescripcionCantidadService(this.ultima_boleta,this.descripcion_operacion_anulacion,this.buscar_anulacion_operacion).subscribe(
	  		res => {
	  			if(res["mensaje"].terminar){
					localStorage.clear();
					this._router.navigate(['/login']);
