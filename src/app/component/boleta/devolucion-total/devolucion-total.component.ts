...
export class DevolucionTotalComponent implements OnInit {
	...
	verificarAnulacionOperacionDos(verificar_dos_nuevo){
	  ...
	  this._usu.guardarAnulacionOperacioneBdServiceDevolucionTotak(this.ultima_boleta,this.descripcion_operacion_anulacion,this.buscar_anulacion_operacion).subscribe(
	  		res => {
	  			if(res["mensaje"].terminar){
					localStorage.clear();
					this._router.navigate(['/login']);
